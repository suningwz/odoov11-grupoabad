...
class OperationsCmcReportXlsx(models.AbstractModel):
    ...
    def _get_lines(self, doc):
        ...
        sql = """
                select  
                b.name, c.name, d.name, sum(a.men_hours), sum(a.worked_hours), sum(a.stop_time_1)
    ...
    def generate_xlsx_report(self, workbook, data, context):
        ...
        sheet.set_column("D:D", 6.57)
        sheet.set_column("E:E", 4.57)
        sheet.set_column("F:F", 4.71)
        sheet.set_column("G:G", 5.57)
        sheet.set_column("H:H", 5.71)
        sheet.set_column("I:I", 6)
        sheet.set_column("J:J", 5)
        sheet.set_column("K:K", 5.86)
        sheet.set_column("L:L", 5.57)
        # Formatos de celda
        number_format = workbook.add_format({'num_format': '#,##0.00', 'size': 8})
        sheet.set_default_row(15)
        # Cabeceras con filtro
        ...
        sheet.write('C5', 'Del ' + context['start_date'] + ' al ' + context['end_date'])
        ...
        sheet.write(6, 3, "Horas Hombre", title)
        sheet.write(6, 4, "Horas Maq.", title)
        sheet.write(6, 5, "Paro Maeq", title)
        sheet.write(6, 6, "Paro Mecan.", title)
        sheet.write(6, 7, "Paro Oper.", title)
        sheet.write(6, 8, "Paro Cliente", title)
        sheet.write(6, 9, "Paro N/I", title)
        sheet.write(6, 10, "Comb. (Gl)", title)
        sheet.write(6, 11, "Grasa", title)
        row = 7
        for c1, c3, c2, c4, c5, c6, c7, c8, c9, c10, c11, c12 in (data):
            sheet.write(row, 0, c1, content)
            sheet.write(row, 1, c2, content)
            sheet.write(row, 2, c3, content)
            sheet.write(row, 3, c4, number_format)
            ...
            sheet.write(row, 11, 'Si' if c12 == 1 else 'No', content)
            row += 1
        row_chart = row
        # Sumar en horas
        ...
        sheet.insert_chart('A%d' % row, chart_1, {'x_offset': 5, 'y_offset': 5, 'x_scale': 0.75, 'y_scale': 1.42})
        # Gráfica 2
        ...
        chart_2.set_style(8)
        sheet.insert_chart('E%d' % row, chart_2, {'x_offset': 5, 'y_offset': 5, 'x_scale': 0.70, 'y_scale': 0.81})
        # Gráfica 3
        chart_3 = workbook.add_chart({'type': 'line'})
        chart_3.add_series({
            'name': "='IO-Diario'!$D$7",
            'categories': "='IO-Diario'!$C$8:$C$%d" % row_chart,
            'values': "='IO-Diario'!$D$8:$D$%d" % row_chart,
        })
        chart_3.add_series({
            'name': "='IO-Diario'!$E$7",
            'categories': "='IO-Diario'!$C$8:$C$%d" % row_chart,
        ...
        sheet.insert_chart('E%d' % (row + 12), chart_3, {'x_offset': 5, 'y_offset': 0, 'x_scale': 0.70, 'y_scale': 0.61})
        o_row = row + 20
        ...
        sheet.merge_range('A%d:L%d' % (o_row + 2, o_row + 5), text, merge_format)
