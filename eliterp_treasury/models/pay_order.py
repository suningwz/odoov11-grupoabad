...
class ListEmployeesOrder(models.Model):
    ...
    def generate_check(self):
        ...
        self.update({'generated': True})  # Cambiamos para no volver a generar cheque
        if not self.pay_order_id.lines_employee.filtered(lambda r: not r.generated):
            self.pay_order_id.update({'state': 'paid'})
        return result

    generated = fields.Boolean('Cheque generado?', defualt=False)
    employee_id = fields.Many2one('hr.employee', 'Nombre de empleado')
    amount = fields.Float('Monto')
    pay_order_id = fields.Many2one('eliterp.pay.order', 'Orden de pago')
...
class PayOrder(models.Model):
    ...
    def _onchange_general_check(self):
        if self.type in ['adq', 'rc'] and self.type_egress == 'bank':
            if self.general_check:
                for line in self.lines_employee:
                    line.update({'generated': True})
            else:
                for line in self.lines_employee:
                    line.update({'generated': False})
