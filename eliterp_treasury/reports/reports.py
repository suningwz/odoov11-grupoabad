...
class AccountsToPayReportExcel(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, context):
        ...
        sheet.set_column("D:D", 6.5)
        sheet.set_column("E:E", 8.5)
        sheet.set_column("F:F", 4.14)
        sheet.set_column("G:G", 6.5)
        sheet.set_column("H:H", 17.14)
        sheet.set_column("I:I", 15.14)
        sheet.set_column("J:J", 20.14)
        sheet.set_column("K:K", 11.57)
        sheet.set_column("K:K", 8.6)
        sheet.set_column("L:L", 8.6)
        sheet.set_column("M:M", 8.6)
        sheet.set_column("N:N", 8.6)
        ...
        sheet.set_default_row(15)
        # Datos
        sheet.merge_range('A1:U2', 'Cuentas por pagar', title)
        sheet.merge_range(2, 11, 2, 15, "VALORES POR VENCER", center_format)
        sheet.merge_range(2, 16, 2, 20, "VALORES VENCIDOS", center_format)
        sheet.merge_range(3, 11, 3, 15, "ANTIGÜEDAD DE CARTERA (DÍAS)", center_format)
        sheet.merge_range(3, 16, 3, 20, "ANTIGÜEDAD DE CARTERA (DÍAS)", center_format)
        ...
        sheet.write(4, 5, "NO.")
        sheet.write(4, 6, "VALOR")
        sheet.write(4, 7, "SALDO PENDIENTE")
        sheet.write(4, 8, "FECHA EMISION")
        sheet.write(4, 9, "FECHA VENCIMIENTO")
        sheet.write(4, 10, "MOROSIDAD")
        sheet.write(4, 11, "1-30")
        sheet.write(4, 12, "31-90")
        sheet.write(4, 13, "91-180")
        sheet.write(4, 14, "181-360")
        sheet.write(4, 15, "MAYOR A")
        sheet.write(4, 16, "1-30")
        sheet.write(4, 17, "31-90")
        sheet.write(4, 18, "91-180")
        sheet.write(4, 19, "181-360")
        sheet.write(4, 20, "MAYOR A")
        row = 5
        for r in data:
            ...
            sheet.write(row, 5, r['amount_note_number'], _right_format)
            sheet.write(row, 6, r['credit_note_number'], _right_format)
            sheet.write(row, 7, r['outstanding_balance'])
            sheet.write(row, 8, r['broadcast_date'], date_format)
            sheet.write(row, 9, r['expiration_date'], date_format)
            sheet.write(row, 10, r['delinquency'], _right_format)
            sheet.write(row, 11, r['overcome_30'], _right_format)
            sheet.write(row, 12, r['overcome_90'], _right_format)
            sheet.write(row, 13, r['overcome_180'], _right_format)
            sheet.write(row, 14, r['overcome_360'], _right_format)
            sheet.write(row, 15, r['overcome_mayor'], _right_format)
            sheet.write(row, 16, r['defeated_30'], _right_format)
            sheet.write(row, 17, r['defeated_90'], _right_format)
            sheet.write(row, 18, r['defeated_180'], _right_format)
            sheet.write(row, 19, r['defeated_360'], _right_format)
            sheet.write(row, 20, r['defeated_mayor'], _right_format)
            row += 1
...
class ChecksIssuedReportPDF(models.AbstractModel):
    ...
    def _get_state(self, voucher):
        if voucher.state == 'draft':
            return 'BORRADOR'
        elif voucher.state == 'cancel':
            return 'ANULADO'
        elif voucher.state == 'posted' and not voucher.reconcile:
            return 'ENTREGADO'
        else:
            return 'CONCILIADO'
    ...
    def _get_lines(self, doc):
        data = []
        arg = []
        if doc['bank_type'] != 'all':
            arg.append(('bank_id', '=', doc['bank_id'].id))
        ...
        for check in checks:
            data.append({
                ...
                'amount': check.amount,
                'state': self._get_state(check.voucher_id)
            })
...
class ChecksIssuedReport(models.TransientModel):
    ...
    start_date = fields.Date('Fecha inicio', required=True)
    end_date = fields.Date('Fecha fin', required=True, default=fields.Date.context_today)
    bank_type = fields.Selection([('all', 'Todos'), ('bank', 'Individual')], 'Tipo de banco', default='all')
    bank_id = fields.Many2one('res.bank', 'Banco', domain=[('type_use', '=', 'payments')])
