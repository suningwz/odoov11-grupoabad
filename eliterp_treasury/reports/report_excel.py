...
class ChecksIssuedXlsx(models.AbstractModel):
    ...
    def _get_lines(self, doc):
        ...
        for check in checks:
            row = []
            row.append(check.bank_id.name)
            row.append(check.date)
            row.append(check.check_date)
            row.append('COEG')
            row.append(check.name)
            row.append(check.voucher_id.concept)
            row.append(check.recipient)
            row.append(check.amount)
            row.append(self._get_state(check.voucher_id))
            data.append(row)
        return tuple(data)
    ...
    def generate_xlsx_report(self, workbook, data, context):
        ...
        sheet.write(2, 2, "FECHA PAGO", bold)
        ...
        row = 3
        col = 0
        for c1, c2, c3, c4, c5, c6, c7, c8, c9 in (data):
            sheet.write(row, col, c1)
            sheet.write(row, col + 1, c2, date_format)
            sheet.write(row, col + 2, c3, date_format)
            sheet.write(row, col + 3, c4)
            sheet.write(row, col + 4, c5)
            sheet.write(row, col + 5, c6)
            sheet.write(row, col + 6, c7)
            sheet.write(row, col + 7, c8, money_format)
...
class CheckVoucher(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, records):
        for record in records:
            ...
            elif (record.bank_id.display_name == 'Banco Internacional 1500619XXX'):
                # Margins
